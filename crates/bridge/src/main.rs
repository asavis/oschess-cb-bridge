...
fn run() -> Result<(), String> {
    ...
    let listener = server::bind(config.port).map_err(|e| format!("port {}: {e}", config.port))?;
    let app = App {
    ...
    println!("oschess bridge {} on http://127.0.0.1:{}", app.version, config.port);
    ...
    server::serve(listener, Arc::new(app)).map_err(|e| e.to_string())
}
